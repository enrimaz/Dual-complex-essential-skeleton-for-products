...
%    \draw (1.5,1.8)node[label=below:$\frac{1}{12}$]{};
%    \draw (6.5,1.8)node[label=below:$\frac{1}{18}$]{};
%    \draw (3.5,2.8)node[label=below:$\frac{1}{6}$]{};
%    \draw (0,3)node[label={[xshift=-1cm, yshift=-0.5cm]$F_1,(2,1)$}]{} --  (4,0)  node[label={[xshift=0cm, yshift=-0.9cm]$F_3,(6,1)$}]{};
%    \draw (4,3) node[label={[xshift=0cm, yshift=-0.1cm]$F,(1,1)$}]{} -- (4,0);
%    \draw (8,3) node[label={[xshift=1cm, yshift=-0.5cm]$F_2,(3,1)$}]{} -- (4,0);
%\end{tikzpicture}
%\end{center} 
...
%Let $X$ be projective over $K$ and let $\Delta = \sum a_i \Delta_i$ be an effective $\mathbb{Q}$-divisor on $X$ with snc support, such that each $a_i \leq 1$. We can extend the definition of the essential skeleton to such a pair:
%\begin{defi}
%The essential skeleton $\Sk(X,\Delta)$ of $(X,\Delta)$ is the union of all skeleta $\Sk(X, \lceil \Delta \rceil , \omega)$, where $\omega$ ranges over all log pluricanonical forms on $X$ with a zero of order at least $r(1-a_i)$ along each $\Delta_i$, where $r$ is the index of the form $\omega$. Call such forms $\Delta$-log pluricanonical forms.
%
%\end{defi}
%
%The reason to define the skeleton this way is that it behaves nicely under birational morphisms: Let $f \colon X'\to X$ be a log resolution. Then there is a $\mathbb{Q}$-divisor $\Gamma$ with snc support, and no coefficient exceeding $1$, such that $K_{X'}+\Gamma' = f^*(K_X+\Delta)$. Take $\Delta'$ to be the positive part of $\Gamma'$.
%\begin{prop}
%Under the identification of the birational points of $X$ with those of $X'$, $\Sk(X, \Delta)$ is identified with $\Sk(X', \Delta')$.
%
%\end{prop}
%\begin{proof}
%Choose log regular models $X_R$ and $X'_R$ so that $f$ extends to a log resolution $f_R \colon X'_R \to X_R$. Let $D$ be the sum of the reduced special divisor of $X_R$ with the closure of $\Delta$, likewise for $D'$. 
%
%%Then $\Sk(X,\Delta)$ is a union of cells closed inside $\Sk{X_R}$
%
%It suffices to check the proposition for divisorial valuations. By Lemma \ref{lskeleton} a divisorial valuation $v$ is in the skeleton $\Sk(X,\Delta)$ if and only if there exists a log regular $R$ model $X_R$ for $(X,\Delta)$ such that, if $D$ is the sum of $\Delta$ with the reduced special divisor, then $v$ is a log canonical center of $(X_R, D)$, and there is an effective $D$-log pluricanonical form whose divisor does not vanish at $v$. As the essential skeleton does not depend on the choice of $R$-model, if this is true for some log regular $R$ model, it is true for every such model.
%
%Now we may choose log regular models $X_R$ and $X'_R$ for $X$ and $X'$ such that $f$ extends to a log resolution $f_R \colon X_R \to X$.
%
%Suppose $v$ is in $\Sk(X, \Delta)$. Then let $\omega$ be a $D$-log pluricanonical form on $X_R$ not vanishing at $v$. As $(X_R, D)$ is dlt, we have that $K_{X'_R}+D'= f_R^*(K_{X_R}+D)+M$ where $M$ is effective. Thus there is a $D'$-log pluricanonical form $\omega'$ on$X_R$, whose divisor of zeros is the pullback of $\text{div}(\omega)+M$. As $v$ is a log canonical center of $(X_R, D)$, $M$ does not vanish along $v$, so neither does $\omega'$. Likewise $v$ is a log canonical center of $(X'_R, D')$.
%
%Conversely, for any $r$, pullback along with multiplication by the divisor of discrepancies induces an isomorphism of vector spaces $H^0(X_R, K_{X_R}+D) \cong H^0(X'_R, K_{X'_R}+D')$. So if $v$ is in $\Sk(X'_R, D')$, it is a log canonical center of $(X'_R, D')$ and there is an effective $D'$-log pluricanonical form $\omega'$ not containing the center of $v$. As a result $v$ is also a log canonical center of $(X_R, D)$, so we need only check the existence of a form on $X_R$ not vanishing at the center of $v$. We take the preimage of $\omega'$ in $H^0(X_R, K_{X_R}+D)$, and this must not vanish along $v$ because its pullback doesn't.
%
%\end{proof}
%
%
%Now suppose $(Z,\Gamma)$ is a dlt pair over $K$, such that $K_Z+\Gamma$ is semiample. Suppose $Z_R$ is a good dlt minimal model, and let $D_Z$ be the reduced special fiber plus $\Gamma$. Take a log resolution $f \colon X \to Z$, and write $K_X+H = f^*(K_Z+\Gamma)$. Take $\Delta$ to be the positive part of $H$. We may embed the open dual complex $\mathcal{D}_0^{=1}(Z_R, D_Z)$ into the birational points of $X$.
%
%\begin{prop}
%This embedding identifies $\mathcal{D}_0^{=1}(Z_R, D_Z)$ with $\Sk(X,H)$.
%
%\end{prop}
%Choose a regular $R$ model $X_R$ for $X$ which is a log resolution of $(Z_R, D_Z)$. As above, it suffices to check the proposition for divisorial valuations. Let $v$ be a divisorial valuation, and suppose $v \in \mathcal{D}_0^{=1}(Z_R, D_Z)$. Then $v$ is a log canonical center for $(Z_R, D_Z)$, so $v$ is also a log canonical center of $(X_R, D)$. For a sufficiently divisible index, we may find a $D_Z$-log pluricanonical form on $Z_R$ whose vanishing locus is $C$, a divisor not containing the center of $v$. After pullback, we get a form whose divisor is supported on the strict transform of $C$ and the exceptional divisors of positive log discrepancy. But none of these contain $v$.
%
%Conversely, if $v$ is in $\Sk(X, H)$, then $v$ is a log canonical center of $(X_R, D)$, so $v$ is a log canonical center of $(Z_R, D_Z)$, hence an element of the open dual complex $\mathcal{D}_0^{=1}(Z_R, D_Z)$.
%
%
%\begin{lemma}\label{smalltoric}
%Let $E$ be the monoid generated by $r_1 \ldots r_{n_1}$, $s_1 \ldots s_{n_2}$ with the single relation $\sum r_i = \sum s_j$. Then any small $\mathbb{Q}$-factorialization of the affine toric variety associated to $E$ is a log resolution.
%
%\end{lemma}
%
%\begin{proof}
%We calculate the fan of $W$. Let $N$ be the dual lattice of $M$. The fan associated to $W$ is the cone of elements of $N\otimes \mathbb{R}$ which are nonnegative on $E$. We consider these as linear functions $l$ on the vector space spanned by the $r_i$ and $s_i$, subject to the restriction that $l(\sum r_i)=l(\sum s_i)$. Let $x_{ij}$ be the function which is $1$ on $r_i$ and $s_j$ and $0$ on all others. Then the fan of $W$ is given by the single cone $C_W$ spanned by the $x_{ij}$.
%
%Now, we any $\mathbb{Q}$-factorialization $\tilde{W}$ corresponds to a simplicial subdivision of the cone $C_W$. For example, take a generic function $g$ on the rays of $C_Z$, then there is a unique subdivision of $C_W$ making $g$ a convex piecewise linear function. It turns out that every choice of $\tilde{W}$ is nonsingular:
%
%A maximal cone of $\tilde{W}$ is spanned by $n=n_1+n_2-1$ independent rays of $C_W$. Each ray of $C_W$ corresponds to a choice of $x_{ij}$, and we can index these by edges of the complete bipartite graph $B$ on the $r_i$ and $s_j$. These $x_{ij}$ are independent if and only if the corresponding edges form a spanning tree. Let $w_1 \ldots w_{n}$ span a maximal cone of $\tilde{W}$.
%
%On this affine chart, $\tilde{W}$ is smooth if and only if the $w_i$ generate $N$ as a lattice. We have shown already that the $x_{ij}$ generate $N$. But every $x_{ij}$ is either one of the $w_i$, or it completes a cycle $B$, so that it is a $\mathbb{Z}$-linear combination of the $w_i$.
%
%\end{proof}
%
%\begin{prop}
%Let $(X, \Delta_X)$ and $(Y, \Delta_Y)$ be semistable good projective dlt minimal models over the germ of a pointed curve $C$. Then the product $(Z, \Delta_Z)$ is log canonical, $K_Z+\Delta_Z$ is semi-ample, and the log canonical centers of $(Z,\Delta_Z)$ are strata of the coefficient $1$ part of $\Delta_Z$..
%\end{prop}
%
%\begin{proof}
%Semi-ample follows because both $K_X+\Delta_X$ and $K_Y+\Delta_Y$ are semi-ample. $Z$ is normal because both $X$ and $Y$ are semistable. 
%
%Let $\tilde{X}$ and $\tilde{Y}$ be log resolutions of $X$ and $Y$, and let $\tilde{Z}$ be a toroidal log resolution of their fs product. In particular, $\tilde{Z}$ is a log resolution of $Z$. Over the generic fiber, $(Z,\Delta_Z)$ is dlt, so we need only compute discrepancies over the special fiber. Let $D$ be such a divisor of $\tilde{Z}$.
%
%Choose log pluricanonical forms $\omega_x$ and $\omega_y$ such each has neither a zero nor a pole along any component of $\Delta_X$ or $\Delta_Y$ respectively, and neither has a zero along the image of $D$.
%
%Let $\omega_z$ be the log pluricanonical form obtained as the wedge product of $\omega_x$ and $\omega_y$. Then $\omega_z$ has neither zeros nor poles along the components of $\Delta_Z$, and does not vanish along the image of $D$.
%
%The sign of the log discrepancy of $D$ is the same as the sign of the weight function of $\omega_z$ at $D$. But $\weight_{\omega_z}(v_1, v_2)=\weight_{\omega_x}(v_1)+\weight_{\omega_y}(v_2)$. Hence $D$ has nonnegative log discrepancy.
%
%Likewise $D$ has log discrepancy $0$ if and only if the weight function of $\omega_z$ evaluates to $0$ at the corresponding valuation. But this requires that both $\omega_x$ and $\omega_y$ have weight $0$ at the corresponding valuations in the skeleta of $\tilde{X}$ and $\tilde{Y}$. Hence the image of $D$ in $(Z,\Delta_Z)$. is a product of strata, hence a stratum of $(Z,\Delta_Z)$.
%\end{proof}
%
%\begin{theorem}
