...
%& = v_{\alpha}(f_x) + v_{\beta}(f_y) \\
%& = v_{\alpha}(\divisor_{\cX^+}(\omega_{\cX_K})) + v_{\beta}(\divisor_{\cY^+}(\omega_{\cY_K})).
%\end{align*}
}
...
\begin{thm}
%Let $\cX^+$ and $\cY^+$ be log-smooth log schemes over $S^+$, let $\cZ^+$ be their $fs$ fibred product. The isomoprhism $\Sk(\cZ^+) \rightarrow \Sk(\cX^+) \times \Sk(\cY^+)$ of skeleta induces a map of Kontsevich-Soibelman skeleta $$\Sk(\cZ_K, \varpi) \rightarrow \Sk(\cX_K,\omega_{\cX_K}) \times \Sk(\cY_K, \omega_{\cX_K}).$$
%\end{thm}
%
%Assuming that the local isomorphism of skeleta $\Sk(\cZ^+) \rightarrow \Sk(\cX^+) \times \Sk(\cY^+)$ is actually a global isomorphism, then it induces an isomorphism of Kontsevich-Soibelman skeleta. Thus, recalling Proposition \ref{prop semistability and iso Kato fans}, we have the following result.
%\begin{cor}  
