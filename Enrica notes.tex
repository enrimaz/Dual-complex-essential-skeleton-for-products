...
\begin{thm}
%Let $\cX^+$ and $\cY^+$ be log-smooth log schemes over $S^+$, let $\cZ^+$ be their $fs$ fibred product. The isomoprhism $\Sk(\cZ^+) \rightarrow \Sk(\cX^+) \times \Sk(\cY^+)$ of skeleta induces a map of Kontsevich-Soibelman skeleta $$\Sk(\cZ_K, \varpi) \rightarrow \Sk(\cX_K,\omega_{X^+}) \times \Sk(\cY_K, \omega_{Y^+}).$$
%\end{thm}
%
